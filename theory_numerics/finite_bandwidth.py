...
def plot_status_map(df: pd.DataFrame, output: Path) -> None:
    ...
    pivot = df.assign(code=df.status_publication.map(code)).pivot(
...
def main() -> None:
    ...
    merged["status_publication"] = merged.apply(classify, axis=1)
    merged.to_csv(args.outdir / "finite_bandwidth_status.csv", index=False)
    plot_status_map(merged, args.outdir / "finite_bandwidth_status.png")
    ...
    with (args.outdir / "unresolved_32segment_search.json").open("w") as handle:
    ...
    for row in old_unresolved.itertuples(index=False):
        ...
        qcqp_rows.append({
            ...
            "status_publication": row.status_publication,
    ...
    qcqp_df.to_csv(args.outdir / "qcqp_sos_unresolved.csv", index=False)
    with (args.outdir / "qcqp_sos_certificates.json").open("w") as handle:
        json.dump(jsonable(sos_payload), handle, indent=2)

    counts = merged.status_publication.value_counts().to_dict()
    unresolved = merged[merged.status_publication == "unresolved"][
        ["sigma_k_over_m", "L"]
    ...
    with (args.outdir / "finite_bandwidth_summary.json").open("w") as handle:
