#!/usr/bin/env python3
"""Reproduce the PRD v1.5 finite-bandwidth protected-sector classification.
...
def plot_status_map(df: pd.DataFrame, output: Path) -> None:
    ...
    pivot = df.assign(code=df.status_prd_v1_5.map(code)).pivot(
...
def main() -> None:
    ...
    merged["status_prd_v1_5"] = merged.apply(classify, axis=1)
    merged.to_csv(args.outdir / "finite_bandwidth_status_prd_v1_5.csv", index=False)
    plot_status_map(merged, args.outdir / "finite_bandwidth_status_prd_v1_5.png")
    ...
    with (args.outdir / "unresolved_32segment_search_prd_v1_5.json").open("w") as handle:
    ...
    for row in old_unresolved.itertuples(index=False):
        ...
        qcqp_rows.append({
            ...
            "status_prd_v1_5": row.status_prd_v1_5,
    ...
    qcqp_df.to_csv(args.outdir / "qcqp_sos_unresolved_prd_v1_5.csv", index=False)
    with (args.outdir / "qcqp_sos_certificates_prd_v1_5.json").open("w") as handle:
        json.dump(jsonable(sos_payload), handle, indent=2)

    counts = merged.status_prd_v1_5.value_counts().to_dict()
    unresolved = merged[merged.status_prd_v1_5 == "unresolved"][
        ["sigma_k_over_m", "L"]
    ...
    with (args.outdir / "finite_bandwidth_prd_v1_5_summary.json").open("w") as handle:
